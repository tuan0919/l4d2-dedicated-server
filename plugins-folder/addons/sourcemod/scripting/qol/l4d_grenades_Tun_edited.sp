...
1.10 (01-Nov-2019)
	...
	- Fixed "GrenadeMenu_Invalid" CPrintToChat not replacing the colors. Thanks to "BHaType" for reporting.
...
public void OnPluginStart()
{
	...
	// ====================================================================================================
	// Translations
	BuildPath(Path_SM, sPath, sizeof(sPath), "translations/l4d2_tun_translation.phrases.txt");
	if (!FileExists(sPath))
		SetFailState("Required translation file is missing: 'translations/l4d2_tun_translation.phrases.txt'");
...
Action Cmd_Reload(int client, int args)
{
	LoadDataConfig();
	if (client)
		CPrintToChat(client, "\x04[\x05Grenade\x04] Reloaded config");
	else
...
void DoSpawnCommand(int client, int args, bool projectile)
{
	...
	if (entity != -1)
	{
		...
		CPrintToChat(client, "\x04[\x05Grenade\x04] \x05Created: \x04%T", translation, client);
	}
}
		...
		case MenuAction_Select:
		{
			...
			if (iWeapon > MaxClients && IsValidEntity(iWeapon))
			{
				...
				if (type)
				{
					...
					// Print
					Format(translation, sizeof(translation), "GrenadeMod_Title_%d", index);
					Format(translation, sizeof(translation), "%t %t", "INFO_TAG", "GrenadeMod_Mode", client, translation, client);
					CPrintToChat(client, translation);
	...
	if (g_bCvarAllow && g_iConfigPrefs != 3 && g_iConfigBinds != 2)
	{
		if (buttons & IN_RELOAD)
		{
			...
			if (GetGameTime() - g_fLastUse[client] > MAX_WAIT)
			{
				...
				{
					int type = IsGrenade(iWeapon);
					if (type != 2)return Plugin_Continue;
					if (type)
					{
						// Cycle through modes
...
void OnNextFrame(int entity)
{
	...
	if (client > 0 && client <= MaxClients && IsClientInGame(client))
	{
		...
		switch (className[0])
		{
			case 'm':
			{
				CPrintToChatAll("%t %t", "INFO_TAG", "Someone has thrown a molotov", client);
				CPrintToChat(client, "%t %t", "INFO_TAG", "Me have thrown a molotov");
			}
			case 'v':
			{
				CPrintToChatAll("%t %t", "INFO_TAG", "Someone has thrown a vomitjar", client);
				CPrintToChat(client, "%t %t", "INFO_TAG", "Me have thrown a vomitjar");
			}
		}
		if (index >= 0 && CheckCommandAccess(client, "sm_grenade", 0) == true && className[0] == 'p')
		{
...
{
	if (index == 0) {
		CPrintToChatAll("%t %t", "INFO_TAG", "Someone has thrown a pipebomb", client);
		CPrintToChat(client, "%t %t", "INFO_TAG", "Me have thrown a pipebomb");
		return entity;
	}
	...
	if (index == 1) {
		CPrintToChatAll("%t %t", "INFO_TAG", "Someone has thrown a grenade", client);
		CPrintToChat(client, "%t %t", "INFO_TAG", "Me have thrown a grenade");
	}
