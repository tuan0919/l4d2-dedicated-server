...
public void OnPluginStart()
{
	...
	Cvar_GlowEnabled = CreateConVar("sm_bwnotice_glow", "0", "Enable making black white players glow?(1/0 = yes/no. Default 1)", FCVAR_NOTIFY);
		...
		case 1: //Survivors
			{
				for(int x = 1; x <= MaxClients; x++)
				{
					...
					else					
					{
						...
						CPrintToChat(x, "%t %t", "DANGER_TAG", "Someone are BnW", target, charName);
...
public void Hook_HealSuccess(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{		
		...
		if (Cvar_Mode.IntValue == 1 || Cvar_Mode.IntValue == 3)   // Show to survivors
		{
			...
			{
				CSkipNextClient(healee);
				CSkipNextClient(healer);
				if(healee != healer)
				{
					// CPrintToChat(x, "\x01\x04[B&W] \x03%N \x01was healed by \x03%N \x01and is no longer Black&White! Thanks!", healee, healer);
					CPrintToChatAll("%t %t", "INFO_TAG", "Someone healed Someone BnW", healee, healer);
					CPrintToChat(healer, "%t %t", "INFO_TAG", "You healed Someone BnW", healee);
					CPrintToChat(healee, "%t %t", "INFO_TAG", "You are healed by Someone", healer);
				}
				else
				{
					CPrintToChatAll("%t %t", "INFO_TAG", "Someone healed themself", healee);
					CPrintToChat(healer, "%t %t","INFO_TAG", "You are healed by yourself");
				}
			}
		}
