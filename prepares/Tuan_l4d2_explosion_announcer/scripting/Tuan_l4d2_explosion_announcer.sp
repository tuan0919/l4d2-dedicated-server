...
// Filenames
// ====================================================================================================
#define CONFIG_FILENAME               "Tuan_l4d2_explosion_announcer"
#define TRANSLATION_FILENAME          "Tuan_l4d2_explosion_announcer.phrases"
...
int g_iLastAttacker[MAXENTITIES + 1];
...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	// LogMessage("class name: %s", classname);
	if (StrEqual(classname, "inferno", false) || StrEqual(classname, "entityflame", false)) {
...
{
	g_iLastAttacker[entity] = GetEntPropEnt(entity, Prop_Data, "m_hOwnerEntity");
	// LogMessage("g_iLastAttacker[%d] = %d", entity, g_iLastAttacker[entity]);
}
...
Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	// if (IsValidClient(attacker)) {
	//     LogMessage("OnTakeDamage victim:[%d]", victim);
	//     LogMessage("OnTakeDamage attacker:[%d]", attacker);
	// }
	return Plugin_Continue;
}
...
{
	// LogMessage("OnKilled output:[%s], caller:[%d], activator: [%d]", output, caller, activator);
	if (!g_bCvar_Enabled)
		return;
...
void OutputMessage(int client, int type)
{
	...
	switch (type)
	{
		case TYPE_GASCAN:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a gascan", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a gascan");
		}
		...
		case TYPE_FUEL_BARREL:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a fuel barrel", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a fuel barrel");
		}
		...
		case TYPE_PROPANECANISTER:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a propane canister", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a propane canister");
		}
		...
		case TYPE_OXYGENTANK:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded an oxygen tank", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded an oxygen tank");
		}
		...
		case TYPE_BARRICADE_GASCAN:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a barricade with gascans", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a barricade with gascans");
		}
		...
		case TYPE_GAS_PUMP:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a gas pump", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a gas pump");
		}
		...
		case TYPE_FIREWORKS_CRATE:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded a fireworks crate", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded a fireworks crate");
		}
		...
		case TYPE_OIL_DRUM_EXPLOSIVE:
		{
			...
			CPrintToChatAll("%t %t", "DANGER_TAG", "Exploded an oil drum", client);
			CPrintToChat(client, "%t %t", "DANGER_TAG", "You exploded an oil drum");
		}
	}
}
